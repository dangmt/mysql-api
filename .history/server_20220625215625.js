...
app.get("/all", async (req, res) => {
  const data = await connection
    .promise()
    .query(
      "Select * from Book as b inner join Chapter as c on c.bookid= b.bookid inner join Problem as p on p.chapterid=c.chapterid inner join Video as v on v.problemid=p.problemid"
    );
  res.json(data[0]);
});
