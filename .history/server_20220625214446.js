...
app.get("/all", async (req, res) => {
  const data = await connection
    .promise()
    .query(
      "Select * from Book as b inner join Chapter as c where c.bookid= b.bookid inner join Problem as p where p.chapterid=c.chapterid inner join Video as v where v.problemid=p.problemid"
    );
  res.json(data);
});
