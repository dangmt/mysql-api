...
app.get("/all", async (req, res) => {
  ...
  const data = await connection
    .promise()
    .query(
      "Select b.name as book,v.name as video, v.numerical_order from Book as b inner join Chapter as c on c.bookid= b.bookid inner join Problem as p on p.chapterid=c.chapterid inner join Video as v on v.problemid=p.problemid order by b.name, v.numerical_order;"
    );
  data[0].forEach((element) => {
    if (!result["Book " + element.book]) {
      result["Book " + element.book] = [];
    }
    result["Book " + element.book].push("Video " + element.video);
  });
  res.json(result);
});
...
app.delete("/chapter/:id", async (req, res) => {
  const data = await connection
    .promise()
    .query("delete from chapterid where chapterid=?;", [req.params.id]);
