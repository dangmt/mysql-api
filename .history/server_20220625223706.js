...
app.get("/all", async (req, res) => {
  ...
  data[0].forEach((element) => {
    ...
    result[element.name].push("Video " + element.name);
  });
  res.json(result);
});
...
app.post("/count", async (req, res) => {
  ...
  const data = await connection
    .promise()
    .query(
      "Select v.name from Book as b inner join Chapter as c on c.bookid= b.bookid inner join Problem as p on p.chapterid=c.chapterid inner join Video as v on v.problemid=p.problemid order by b.name, v.numerical_order;"
    );
  data[0].forEach((element) => {
    if (!result[element.name]) {
      result[element.name] = [];
